...
1. **進入測驗前不知道要花多久、會遇到什麼題型。** 原流程從 CTA 直接跳到第 1 題，使用者無法先建立預期。已增加 35 題、約 8–10 分鐘、自動保存、能量題規則與「結果不是定論」的簡短說明。
...
- 首次 `/assessment` 顯示 35 題、8–10 分鐘、自動保存與能量題說明。
...
- 仍建議在實體 iPhone／Android 各跑一次 35 題與結果頁；本次變更沒有新增寬版固定尺寸元件。
