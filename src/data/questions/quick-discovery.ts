...
const situationalQuestions: Question[] = [
  choice('SJT01', 'situational_choice', '一件你正在做的重要事情突然卡住，而且你還不知道原因。你通常第一個會做什麼？', [
    talentOption('analytical_reasoning', '拆開問題，逐一排除可能原因'),
    talentOption('emotional_perception', '先留意相關的人是否焦慮或有所保留'),
    talentOption('initiative', '先做一個低風險嘗試，讓事情重新動起來'),
    talentOption('precision', '回頭核對事情的要求、已有紀錄與執行細節'),
  ]),
  choice('SJT02', 'situational_choice', '你接手一件做到一半、相關資訊散落各處的事情。第一步通常是？', [
    talentOption('pattern_recognition', '比對過去紀錄，找出反覆出現的脈絡'),
    talentOption('communication', '問相關的人，各自目前知道什麼、期待什麼'),
    talentOption('planning', '整理已完成、互相影響的部分與接下來的順序'),
    talentOption('adaptability', '先接住眼前最急的變化，再動態調整'),
  ]),
  choice('SJT03', 'situational_choice', '能用的時間和材料有限，四件事都有人認為最重要。你會先怎麼處理？', [
    talentOption('quantitative_reasoning', '把成本、效益與風險轉成可比較數字'),
    talentOption('influence', '理解大家在意什麼，組織一個能促成共識的主張'),
    talentOption('prioritization', '先訂判準，明確決定哪些暫時不做'),
    talentOption('persistence', '選定一條路後持續排除障礙，避免反覆改向'),
  ]),
  choice('SJT04', 'situational_choice', '第一次接觸一個完全陌生的新點子，你最想先從哪裡理解？', [
  ...
  choice('SJT05', 'situational_choice', '幾個人要一起完成一件事，但需要做到什麼還很模糊，大家的立場也不同。你會先？', [
    talentOption('learning_agility', '快速補齊陌生知識，先聽懂每個人的說法'),
...
const forcedChoiceQuestions: Question[] = [
  choice('FC01', 'forced_choice', '如果只能選一種貢獻方式，你比較願意成為哪一種角色？', [
    ...
    talentOption('creative_ideation', '提供別人還沒想過的方向'),
  ]),
  choice('FC02', 'forced_choice', '時間只夠改善一件事，你會優先強化哪一部分？', [
    talentOption('precision', '成果的正確性與一致性'),
    talentOption('coordination', '一起做事時的前後銜接與同步'),
    talentOption('learning_agility', '大家理解新工具或新知的速度'),
    talentOption('influence', '重要想法被採納的可能'),
  ]),
  choice('FC03', 'forced_choice', '兩種做法都能完成事情，但只能保留一種。你較難放棄的是？', [
    talentOption('planning', '事前排好階段與先後關係'),
  ...
  choice('FC04', 'forced_choice', '幾個人長期一起做事時，你最願意持續承擔哪種責任？', [
  ...
  choice('FC05', 'forced_choice', '一件事的定義不清，大家又有歧見時，你比較相信哪種切入方式？', [
    talentOption('structuring_ambiguity', '先建立問題框架與邊界'),
    talentOption('conflict_navigation', '先處理彼此真正的分歧'),
...
const behaviorQuestions: Question[] = [
  scaled('BEH01', 'behavior', '最近三個月，別人最常因為下列哪種行為找你幫忙？', [
    talentOption('planning', '把想做成的事拆成步驟並安排順序'),
  ...
  scaled('BEH02', 'behavior', '正在做的事情突然出現變化時，哪種反應最常出現在你身上？', [
  ...
  scaled('BEH03', 'behavior', '同時有好幾件事需要處理時，哪種行為最像你的日常？', [
    talentOption('prioritization', '明確排序，也接受有些事情暫不處理'),
    talentOption('coordination', '確認每個人正在做的部分能順利接上'),
    talentOption('persistence', '固定推進最關鍵的一項直到跨過障礙'),
    talentOption('quantitative_reasoning', '用需要多少時間或可能影響來估算怎麼分配'),
  ]),
  scaled('BEH04', 'behavior', '幾個人一起討論重要事情時，哪一種貢獻最常由你自然做出？', [
...
const evidenceQuestions: Question[] = [
  ...
  scaled('EVD04', 'evidence', '哪一類成果最容易讓你提出完整的前因、做法與結果？', [
    talentOption('verbal_reasoning', '釐清說法，使文字或決定更嚴謹'),
    talentOption('emotional_perception', '看出未明說需求，改善互動或提供的協助'),
    talentOption('prioritization', '透過取捨把有限時間與力氣放到高影響處'),
...
const positiveEnergyQuestions: Question[] = [
  ...
  choice('ENG03', 'energy', '忙碌一天中，哪種活動片段最可能讓你恢復動力？', [
  ...
  choice('ENG04', 'energy', '如果能自由選擇接下來要做的事，你較期待哪一種？', [
    energyOption('verbal_reasoning', '琢磨文字、論點與精確表達', 1),
    energyOption('influence', '整理一套能爭取支持的說法', 1),
    energyOption('coordination', '讓不同的人與需要用到的東西順利配合', 1),
...
const drainingEnergyQuestions: Question[] = [
  ...
  choice('ENG08', 'energy', '壓力很大時，哪類事情最可能額外消耗你的能量？', [
    energyOption('quantitative_reasoning', '密集處理數據與精算差異', -1),
    energyOption('teaching_coaching', '反覆配合不同人的學習速度', -1),
    energyOption('prioritization', '持續替不同需求做困難取捨', -1),
    energyOption('adaptability', '頻繁改變方向與做事節奏', -1),
  ]),
  choice('ENG09', 'energy', '和別人一起處理重要事情時，哪種負責內容即使表現不差，事後仍最容易疲累？', [
    energyOption('verbal_reasoning', '逐字確認定義與論述精確性', -1),
    energyOption('influence', '說服重要的人改變決定', -1),
    energyOption('coordination', '同時追蹤多人答應的事與前後銜接', -1),
    energyOption('persistence', '在反覆受阻後繼續投入', -1),
  ]),
  choice('ENG10', 'energy', '面對下列複雜活動，哪一種最容易讓你覺得腦力透支？', [
...
const interestQuestions: Question[] = [
  ...
  choice('INT03', 'interest', '如果能參加一場實作體驗，你會先選？', [
    interestOption('quantitative_reasoning', '資料分析與決策'),
    interestOption('influence', '說服、協商與行為影響'),
    interestOption('prioritization', '困難取捨與有限時間安排'),
    interestOption('persistence', '長期挑戰與韌性訓練'),
  ]),
  ...
  choice('INT05', 'interest', '你最想觀察哪一種熟練的人如何處理事情？', [
    ...
    interestOption('conflict_navigation', '能讓激烈分歧重新展開對話的人'),
  ]),
];

const environmentQuestions: Question[] = [
  choice('ENV01', 'environment', '如果每天都要投入一段固定時間，你更希望節奏接近哪一種？', [
    dimensionOption('steady_pace', '可預期、能長時間專注的節奏', 'environmentSignals'),
    dimensionOption('varied_pace', '內容多變、需要頻繁切換的節奏', 'environmentSignals'),
    dimensionOption('sprint_pace', '短期高強度、完成後明確收尾', 'environmentSignals'),
    dimensionOption('self_paced', '能依自己的狀態安排快慢', 'environmentSignals'),
  ]),
  choice('ENV02', 'environment', '和別人一起做事時，你較容易穩定發揮的方式是？', [
    dimensionOption('independent', '大部分獨立完成，必要時同步', 'environmentSignals'),
    dimensionOption('small_team', '與固定的幾個人密切合作', 'environmentSignals'),
    dimensionOption('cross_functional', '經常串聯不同專長的人', 'environmentSignals'),
    dimensionOption('public_facing', '大量與不熟悉的人、社群或外部對象互動', 'environmentSignals'),
  ]),
  choice('ENV03', 'environment', '一件事有固定規則時，你偏好哪一種狀態？', [
  ...
  choice('ENV04', 'environment', '哪種活動場景最接近你願意長期承受的狀態？', [
    ...
    dimensionOption('mobile_predictable', '經常移動，但要做的事和風險相對明確', 'environmentSignals'),
  ...
  choice('ENV05', 'environment', '如果成果同樣重要，你更自然採用哪種貢獻模式？', [
    ...
    dimensionOption('team_integrator', '串聯其他人並協助大家共同前進', 'workStyleSignals'),
...
const valueQuestions: Question[] = [
  choice('VAL01', 'values', '想像兩個未來方向的條件相近，哪一項最可能成為你的決定因素？', [
    dimensionOption('mastery', '能長期累積某項能力的深度', 'valueSignals'),
  ...
  choice('VAL02', 'values', '如果一件重要事情只能保住一項特色，你比較想保留？', [
  ...
  choice('VAL03', 'values', '哪一種肯定最容易讓你覺得投入很值得？', [
    dimensionOption('expert_recognition', '專業能力受到信任', 'valueSignals'),
    dimensionOption('user_outcome', '接受成果的人真的因此變好', 'valueSignals'),
    dimensionOption('team_success', '一起做事的人因你的貢獻更順利', 'valueSignals'),
    dimensionOption('visible_ownership', '你完整主導並留下清楚成果', 'valueSignals'),
  ]),
  choice('VAL04', 'values', '選擇下一個重要機會時，你最不希望犧牲的是？', [
    ...
    dimensionOption('international_collaboration', '與不同文化背景的人合作', 'valueSignals'),
  ]),
  choice('VAL05', 'values', '回顧一段讓你滿意的經驗，你最希望它留下什麼？', [
