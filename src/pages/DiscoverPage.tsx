...
export function DiscoverPage() {
  ...
  return <main className="overflow-hidden">
    <section className="relative isolate border-b border-ink/10">
      ...
      <div className="editorial-shell grid min-h-[calc(100svh-76px)] items-center gap-10 py-16 lg:grid-cols-[1.18fr_.82fr] lg:py-24">
        <div className="relative z-10 reveal-block">
          ...
          <p className="mt-8 max-w-2xl text-base leading-8 text-ink/65 sm:text-lg">從你的日常選擇、思考方式與真實經驗中，找出你可能從沒意識到的天賦，並探索真正適合你的工作方式與職涯方向。</p>
    ...
    <section className="section-space"><div className="editorial-shell">
      <SectionIntro number="03" eyebrow="Career Match Preview" title="推薦一份工作，也說清楚為什麼。" body="Fit 不是結論。你會看見支持理由、可能摩擦、信心程度，以及走到那裡的距離。" />
    ...
    <section className="section-space"><div className="editorial-shell grid gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center"><div className="experiment-visual" aria-hidden="true"><span className="experiment-number">03</span><div className="experiment-note note-one">訪談一位從業者</div><div className="experiment-note note-two">做一份迷你產出</div><div className="experiment-note note-three">記錄投入後的能量</div></div><SectionIntro number="06" eyebrow="Career Experiment" title="不用立刻轉職，先花幾小時靠近它。" body="每個方向都有一個低風險實驗。你可以做出小作品、訪談從業者，或模擬核心任務，再用真實感受修正下一步。" /></div></section>
...
function CareerPreview({ title, en, score, reasons, friction, featured = false }: { title: string; en: string; score: string; reasons: string[]; friction: string; featured?: boolean }) { return <article className={`career-preview reveal-block ${featured ? 'career-preview-featured' : ''}`}><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold tracking-[.16em] text-ink/40 uppercase">Career Fit Index · 示意</p><h3 className="mt-4 text-2xl font-semibold sm:text-3xl">{title}</h3><p className="text-sm text-ink/45">{en}</p></div><div className="preview-score">{score}<small>/100</small></div></div><ul className="mt-9 space-y-3">{reasons.map((reason) => <li key={reason}>↳ {reason}</li>)}</ul><div className="mt-auto pt-10"><p className="border-t border-ink/10 pt-4 text-sm text-ink/55"><b>Potential friction</b><br />{friction}</p></div><p className="mt-3 text-xs leading-5 text-ink/45">吻合指數用於比較目前資料庫中的相對重疊，不是成功機率或考試分數。</p></article>; }
