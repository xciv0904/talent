...
import { buildCareerDirections, buildDynamicTieBreaker, buildExplorationPriority } from '../engine';
import {
...
export function ResultsPage() {
  ...
  return <main className="mx-auto max-w-7xl px-5 py-12 sm:py-20">
    <header className="max-w-4xl"><p className="text-sm font-bold tracking-[.2em] text-slate-600 uppercase">Your Career Navigator · Beta</p><h1 className="mt-4 text-4xl font-semibold tracking-tight sm:text-6xl">先找到值得探索的方向，再用一個小行動確認。</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600">這不是替你決定職業。系統把既有分析整理成探索優先順序，讓你今天就知道可以先做什麼。</p></header>
    ...
    <div className="mt-16 space-y-20">
      ...
      <section>{sectionTitle('03', '先比較這幾種工作活動')}<div className="mb-7 max-w-3xl rounded-3xl bg-slate-100 p-5"><p className="text-sm font-bold text-slate-600">Decision Clarity · {priority ? decisionClarityLabels[priority.decisionClarity] : '計算中'}</p><p className="mt-2 leading-7">{priority?.interpretation}</p><p className="mt-2 text-sm leading-6 text-slate-600">Career Fit 是 60 種工作與你目前 Profile 的相對吻合指標，不是成功機率、錄取機率、能力百分比或考試分數。</p></div><p className="mb-7 max-w-3xl leading-7 text-slate-600">{evidenceBacked ? '我們先把相近的工作活動聚在一起。差距在同一 proximity cluster 的方向會使用相同優先層級，不製造不存在的第一、第二名。' : '目前 Career Match 的可追溯證據不足，系統不會說你什麼都不適合，也不會假裝知道哪個最好。先比較工作活動，再用 20 分鐘體驗收集新證據。'}</p><div className="grid gap-5 lg:grid-cols-3">{directions.map((direction) => <DirectionCard key={direction.id} direction={direction} selected={selected?.id === direction.id} onSelect={() => { selectCareerDirection(direction.id); setShowGuided(false); }} />)}</div></section>

      <section id="direction-choice" className="scroll-mt-24 rounded-[2rem] bg-blue-50 p-6 sm:p-10">{sectionTitle('04', '哪個方向讓你最想多了解一點？')}<p className="max-w-3xl text-slate-600">先選一個願意花 20 分鐘了解的方向即可。你之後隨時可以換。</p><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{directions.map((direction) => <button key={direction.id} type="button" aria-pressed={selected?.id === direction.id} onClick={() => { selectCareerDirection(direction.id); setShowGuided(false); }} className={`rounded-2xl border p-4 text-left font-semibold ${selected?.id === direction.id ? 'border-slate-950 bg-slate-950 text-white' : 'border-slate-300 bg-white'}`}>{direction.title}</button>)}<button type="button" onClick={() => { saveNavigatorNeed('guided_direction'); selectCareerDirection(null); setShowGuided(true); }} className="rounded-2xl border border-dashed border-slate-400 bg-white p-4 text-left font-semibold">我還是不知道</button></div>
