...
const categories: Array<[CareerTab, string]> = [['all', '全部方向'], ['best_fit', 'Best Fit'], ['easier_transition', 'Easier Transition'], ['high_potential', 'High Potential'], ['surprise_me', 'Surprise Me']];
...
export function CareersPage() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-12 sm:px-5 sm:py-16"><header className="max-w-3xl"><h1 className="font-serif text-[clamp(3rem,7vw,5.5rem)] font-normal leading-none tracking-[-.04em]">Career Matches</h1><p className="mt-5 text-lg text-ink/60">四個分類使用不同邏輯，不是同一份排行榜換標題；「全部方向」只呈現 engine 原始排序。</p><p className="mt-3 text-sm leading-6 text-ink/50">目前結果是在本站收錄的 60 個職業中比較較吻合的方向。沒有出現在推薦中，不代表你不適合；真實職涯選項比目前資料庫更廣。</p></header><div className="mt-9 flex gap-2 overflow-x-auto pb-3 [scrollbar-width:none]">{categories.map(([key, label]) => <button key={key} type="button" onClick={() => setActive(key)} className={`result-tab min-h-11 shrink-0 rounded-full px-5 text-sm font-semibold ${active === key ? 'bg-ink text-white' : 'border border-ink/10 bg-white/70'}`}>{label} · {key === 'all' ? careerResults.matches.length : careerResults.categories[key].length}</button>)}</div><div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{matches.map((match) => <CareerCard key={match.careerId} match={match} />)}</div>{matches.length === 0 && <p className="mt-7 rounded-3xl border border-dashed border-ink/20 p-7 text-ink/60">沒有符合這個分類門檻的結果；系統不會為了填滿卡片降低條件。</p>}</main>;
}
