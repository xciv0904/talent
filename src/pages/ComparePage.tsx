...
export function ComparePage() {
  ...
  return <main className="mx-auto max-w-5xl px-5 py-14"><h1 className="text-5xl font-semibold">比較職涯</h1><p className="mt-4 text-slate-600">Fit 相近的工作，可能在環境摩擦與準備需求上完全不同。</p><div className="mt-9 grid grid-cols-1 gap-3 sm:grid-cols-2"><CareerSelect value={leftId} onChange={setLeftId} candidates={candidates} /><CareerSelect value={rightId} onChange={setRightId} candidates={candidates} /></div>{left && right && leftCareer && rightCareer && <div className="mt-6 overflow-x-auto rounded-3xl border border-slate-200 bg-white"><div className="min-w-[42rem]"><CompareRow label="職涯" left={leftCareer.titleZh} right={rightCareer.titleZh} strong /><CompareRow label="Career Fit index" left={formatFitIndex(left.matchScore)} right={formatFitIndex(right.matchScore)} /><CompareRow label="Confidence" left={left.confidence} right={right.confidence} /><CompareRow label="職業通常門檻" left={careerEntryBarrierLabel[leftCareer.entryBarrier]} right={careerEntryBarrierLabel[rightCareer.entryBarrier]} /><CompareRow label="常見起點" left={careerRequirementSummary(leftCareer)} right={careerRequirementSummary(rightCareer)} /><CompareRow label="個人 Entry Distance" left="尚未估算" right="尚未估算" /><CompareRow label="Talent Match index" left={formatFitIndex(left.talentMatch)} right={formatFitIndex(right.talentMatch)} /><CompareRow label="Interest Match index" left={formatFitIndex(left.interestMatch)} right={formatFitIndex(right.interestMatch)} /><CompareRow label="Environment Match index" left={formatFitIndex(left.environmentMatch)} right={formatFitIndex(right.environmentMatch)} /><CompareRow label="主要摩擦" left={left.potentialFrictions[0] ?? '無明顯訊號'} right={right.potentialFrictions[0] ?? '無明顯訊號'} /></div></div>}<p className="mt-4 text-sm leading-6 text-slate-500">「職業通常門檻」來自職業資料庫的一般要求；個人 Entry Distance 需另行比對教育、技能、經驗、證照與作品集，不會影響 Career Fit。</p></main>;
}
