...
import { entryDistanceLabel, formatFitIndex } from '../utils';
...
export function ComparePage() {
  ...
  return <main className="mx-auto max-w-5xl px-5 py-14"><h1 className="text-5xl font-semibold">比較職涯</h1><p className="mt-4 text-slate-600">Fit 相近的工作，可能在環境摩擦與進入距離上完全不同。</p><div className="mt-9 grid grid-cols-2 gap-3"><CareerSelect value={leftId} onChange={setLeftId} candidates={candidates} /><CareerSelect value={rightId} onChange={setRightId} candidates={candidates} /></div>{left && right && <div className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white"><CompareRow label="職涯" left={title(left.careerId)} right={title(right.careerId)} strong /><CompareRow label="Career Fit index" left={formatFitIndex(left.matchScore)} right={formatFitIndex(right.matchScore)} /><CompareRow label="Confidence" left={left.confidence} right={right.confidence} /><CompareRow label="Entry Distance" left={entryDistanceLabel[left.entryDistance.level]} right={entryDistanceLabel[right.entryDistance.level]} /><CompareRow label="Talent Match index" left={formatFitIndex(left.talentMatch)} right={formatFitIndex(right.talentMatch)} /><CompareRow label="Interest Match index" left={formatFitIndex(left.interestMatch)} right={formatFitIndex(right.interestMatch)} /><CompareRow label="Environment Match index" left={formatFitIndex(left.environmentMatch)} right={formatFitIndex(right.environmentMatch)} /><CompareRow label="主要摩擦" left={left.potentialFrictions[0] ?? '無明顯訊號'} right={right.potentialFrictions[0] ?? '無明顯訊號'} /></div>}</main>;
}
